...
/*
 * GameHelper contains some game logic to simplify GameScene
 */
public class GameHelper
{
	public static void reset(Board b, TileSprite [] tiles, int size)
	{
		...
		for(int i = 0; i < b.count; i++ ) {
			int j = 2 * RandomService.getInt(b.count / 2);
			int k = 2 * RandomService.getInt(b.count / 2);
			tmp = bs[j]; bs[j] = bs[k]; bs[k] = tmp;
		}
		...
		System.out.println("RESET  BOARD: " +b.w + "," + b.h);
	}
